 ...
 *
 */
public class SaleuiX {// 红冲并复制和总经理的编辑界面
	...
	public void display(String Job, String name, String ID,
		...
		try {
			...
			root = FXMLLoader.load(getClass().getResource("SaleuiX.fxml"));
		...
		for (int i = 0; i < 6; i++) {
			...
			for (int j = 0; j < mylist2.size(); j++) {
				com[i].getItems().add(mylist2.get(i).getName());
			}
	...
	private void HandleTI(ActionEvent event) {
		...
		sales.setLineItem(mylist);
		sales.setVoucher(sales.getVoucher());
		...
		if (isRedCopy) {// 为红冲并复制服务
			...
			sales.setVoucherReceived(new ArrayList<>());
			sales.setLineItemReceived(new ArrayList<>());
			...
			sales.setVoucherReceived(sales.getVoucherReceived());
			sales.setLineItemReceived(sales.getLineItemReceived());
			sales.setSalesType(SalesReceiptType.SALES);
			try {
				ResultMessage message = helper.getService()
						.completeModifySales(sales, id);
