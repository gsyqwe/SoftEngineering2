 ...
 *
 * 
 * @author 82646 关于总价的问题，都应该有总价这一栏，点击总价应该在界面上算出来当前的总价
 * 
 *
 */

public class InBack {// 制定进货退货单
	...
	public void display(String Job, String name, String ID,
		...
		InBack sal = new InBack();
		j = Job;
		id = ID;
		n = name;
		sal.display(Job, name, ID);
		sales = SaReceipt;
		sales.setSalesType(SalesReceiptType.PURCHASE_RETURN);
	...
	public void display(String Job, String name, String ID) {
		...
		try {
			...
			root = FXMLLoader.load(getClass().getResource("InBack.fxml"));
	...
	private void HandleSave(ActionEvent event) {
		...
		if (date == null || date.equals("")) {
			...
			String warning = "The date cannot be empty even when you save the receipt.";
		...
		sales.setSalesType(SalesReceiptType.PURCHASE_RETURN);// 设定单据的类型：进货单
	...
	private void HandleTI(ActionEvent event) {
		...
		try {
			...
			sales.setSalesType(SalesReceiptType.PURCHASE_RETURN);
			ResultMessage message = helper.getService().completeAdd(sales);
	...
	private ArrayList<MemberVO> getAllMember() {
		...
		try {
			...
			ArrayList<MemberVO> memberList = helper.getService().getNowMembers(
					MemberType.SUPPLIER);// 这里面已经把欠钱过多的member剔除在名单外了
