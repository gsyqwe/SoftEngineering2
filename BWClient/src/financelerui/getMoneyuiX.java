...
public class getMoneyuiX {// 通过红冲或者总经理的编辑来到这个界面
	...
	private static ArrayList<MemberVO> memberlist = new ArrayList<>();// memberlist

	private static BankAccountVO bankaccount;// bankaccount

	private static MemberVO member;// 客户

	private static ArrayList<LineItemVO> lineItems = new ArrayList<>();// LineItemVO

	private static ArrayList<AccountName> accountnamelist = new ArrayList<>();// AccountName
	...
	FinancialReceiptVO f1) {
		...
		try {
			...
			root = FXMLLoader.load(getClass().getResource("getMoneyuiX.fxml"));
		...
		getMoneyuiX getmon = new getMoneyuiX();
