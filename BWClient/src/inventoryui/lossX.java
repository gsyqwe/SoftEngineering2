...
public class lossX {// 红冲或者总经理的编辑来这个界面
		...
		for (int i = 0; i < lineItems.size(); i++) {
			...
			if (i == 0) {
				...
				for (int j = 0; j < commoditylist.size(); j++) {
					if (commoditylist.get(j).getId()
							.equals(lineItems.get(i).getId())) {
						c1.setValue(commoditylist.get(j).getName());
					}
				}
				...
				for (int j = 0; j < commoditylist.size(); j++) {
					if (commoditylist.get(j).getId()
							.equals(lineItems.get(i).getId())) {
						c2.setValue(commoditylist.get(j).getName());
					}
				}
				...
				for (int j = 0; j < commoditylist.size(); j++) {
					if (commoditylist.get(j).getId()
							.equals(lineItems.get(i).getId())) {
						c3.setValue(commoditylist.get(j).getName());
					}
				}
				tx[i].setText("" + lineItems.get(i).getQuantity());
				...
				for (int j = 0; j < commoditylist.size(); j++) {
					if (commoditylist.get(j).getId()
							.equals(lineItems.get(i).getId())) {
						c4.setValue(commoditylist.get(j).getName());
					}
				}
	...
	public void display(String Job, String name, String ID) {
		...
		try {
			...
			root = FXMLLoader.load(getClass().getResource("lossX.fxml"));
	...
	private void HandleTI(ActionEvent event) throws RemoteException,
		...
		vo.setInventoryType(InventoryReceiptType.BREAKAGE);
